...
const ScoreLogic = {
  ...
  // examUnit = { name, max, items: [{name, max}], indicators }
  // เหมือน unit เลย — items คือสอบย่อย, max คือคะแนนเต็มสอบ (scaledMax)
  getExamMax: t => {
    const eu = App.examUnits?.[t];
    if (eu && eu.length > 0)
      return eu.reduce((s,u) => s + (Number(u.max)||0), 0);
    return Math.max(0, ScoreLogic.getTarget() - ScoreLogic.getUnitsMax(t));
  },
  getExamRawMax: (t, ei) =>
    ((App.examUnits?.[t]?.[ei])?.items||[]).reduce((s,i) => s+(Number(i.max)||0), 0),
  // คำนวณคะแนนสอบรวมหลัง scale (เหมือน calcKeepFromFlat)
  calcExamScaled: (t, examFlatVals) => {
    let total = 0, offset = 0;
    (App.examUnits?.[t]||[]).forEach(eu => {
      const items  = eu.items || [];
      const rawMax = items.reduce((s,i) => s+(Number(i.max)||0), 0);
      let raw = 0;
      items.forEach((_, i) => { raw += Number(examFlatVals[offset+i])||0; });
      total  += ScoreLogic.calcScaled(raw, rawMax, Number(eu.max)||0);
      offset += items.length;
    });
  ...
  calcScaled: (raw, rawMax, scaledMax) => {
    ...
    const result = Math.round((raw / rawMax) * scaledMax * 100) / 100;
  ...
  calcKeepFromFlat: (t, flatVals) => {
    ...
    ScoreLogic.getUnitOffsets(t).forEach(({ unitIndex, start, count }) => {
      ...
      const scaled = ScoreLogic.calcScaled(raw, rawMax, Number((App.units[t]||[])[unitIndex]?.max) || 0);
      total += isNaN(scaled) ? 0 : scaled;
    });
...
function refreshUnitPanelLabels() {
  if ($('sum_t1_units')?.parentElement) $('sum_t1_units').parentElement.innerHTML = `<span>คะแนนรวมทุกหน่วย เทอม 1:</span><strong id="sum_t1_units">${ScoreLogic.getUnitsMax(1)}</strong><span>/</span><span class="res">${ScoreLogic.getTarget()}</span>`;
  if ($('sum_t2_units')?.parentElement) $('sum_t2_units').parentElement.innerHTML = `<span>คะแนนรวมทุกหน่วย เทอม 2:</span><strong id="sum_t2_units">${ScoreLogic.getUnitsMax(2)}</strong><span>/</span><span class="res">${ScoreLogic.getTarget()}</span>`;
      ...
      : exams.map((exam, ei) => {
          ...
          return `<div class="sub-row p-3 border rounded bg-white mb-2" style="display:block;border-color:#bae6fd;">
            <div class="d-flex align-items-center gap-2 flex-wrap mb-2">
              <span style="font-weight:800;font-size:.88rem;color:#0369a1;">สอบ ${ei+1}.</span>
              <input class="sub-name" type="text" value="${exam.name||''}" placeholder="ชื่อการสอบ"
                onchange="App.examUnits[${t}][${ei}].name=this.value">
              <span class="sub-max-lbl">คะแนนเต็มสอบ</span>
              <input class="sub-max" type="number" min="1" value="${Number(exam.max)||0}"
                onchange="App.examUnits[${t}][${ei}].max=Number(this.value)||0;updateAutoScoreDisplay();renderSubList(${t});">
              <button class="btn-rm" onclick="rmExam(${t},${ei})">✕</button>
            </div>

            <div style="font-weight:700;font-size:.82rem;color:#374151;margin-bottom:6px;">📋 สอบย่อย</div>
            <div class="ms-2" style="display:flex;flex-direction:column;">
              ${(exam.items||[]).map((item, ii) => `
                <div class="d-flex align-items-center gap-2 flex-wrap" style="padding:8px 0;border-bottom:1px solid #f1f5f9;">
                  <input class="sub-name" type="text" value="${item.name||''}" placeholder="ชื่อสอบย่อย"
                    onchange="App.examUnits[${t}][${ei}].items[${ii}].name=this.value">
                  <span class="sub-max-lbl">เต็ม</span>
                  <input class="sub-max" type="number" min="1" value="${Number(item.max)||0}"
                    onchange="App.examUnits[${t}][${ei}].items[${ii}].max=Number(this.value)||0;renderSubList(${t});">
                  <button class="btn-rm" onclick="rmExamItem(${t},${ei},${ii})">✕</button>
                </div>`).join('')}
              <button type="button" class="btn-add-sub mt-2" onclick="addExamItem(${t},${ei})">＋ เพิ่มสอบย่อย</button>
            </div>
            <div class="raw-preview mt-2">คะแนนดิบรวมสอบย่อย: <strong>${ScoreLogic.getExamRawMax(t,ei)}</strong> | คะแนนที่เก็บเข้าสอบ: <strong class="res">${Number(exam.max)||0}</strong></div>

            ${allInds.length > 0 ? `
            <div style="margin-top:10px;border-top:2px dashed #bae6fd;padding-top:10px;">
              <div style="font-weight:700;font-size:.82rem;color:#0369a1;margin-bottom:6px;">
                🎯 ตัวชี้วัด <span style="font-weight:400;color:#94a3b8;">(${selInds.length}/${allInds.length})</span>
              </div>
              <div style="max-height:200px;overflow-y:auto;">${indHtml}</div>
            </div>` : ''}
          </div>`;
        }).join('');
...
function addExam(t) {
  ...
  const n = App.examUnits[t].length + 1;
  App.examUnits[t].push({ name: `สอบครั้งที่ ${n}`, max: ScoreLogic.getExamMax(t)||10, items: [{ name: 'สอบย่อย 1', max: 10 }], indicators: [] });
  if (!App.activeSubUnit) App.activeSubUnit = {};
...
async function loadGrades() {
  ...
  try {
    ...
    const _isMath = ['ม.1','ม.2','ม.3'].some(c => String(App.loadedClass||'').startsWith(c));
    if ($('btnPp6T1'))   $('btnPp6T1').style.display   = '';
    if ($('btnPp6T2'))   $('btnPp6T2').style.display   = _isMath ? '' : 'none';
    if ($('btnPp6Year')) $('btnPp6Year').style.display  = _isMath ? 'none' : '';
    if ($('btnPp6Sem'))  $('btnPp6Sem').style.display   = 'none';
    // ซ่อนแท็บ "ทั้งปี" สำหรับมัธยม
    const termTabYear = $('termTabYear');
    if (termTabYear) {
      termTabYear.style.display = _isMath ? 'none' : '';
      // ถ้าตอนนี้อยู่ที่ tab ทั้งปี และเป็นมัธยม ให้สลับกลับเทอม 1
      if (_isMath && App.activeTerm === 0) switchGradeTerm(1);
    }
    document.dispatchEvent(new Event('subjLoaded'));
...
function buildHead() {
  ...
  if (isYear) {
    ...
    if (uIdx === null) {
      ...
      // สอบ — จบแค่รวมสอบ
      const examUnits = App.examUnits?.[t] || [];
      const totalItems = examUnits.reduce((s,eu) => s+(eu.items||[]).length, 0);
      if (examUnits.length > 0) {
        r1 += `<th colspan="${totalItems + examUnits.length}" class="th-t${t}s sc${t}">📝 รายการสอบ เทอม ${t}</th>`;
        examUnits.forEach(eu => {
          const c = (eu.items||[]).length;
          if (c > 0) {
            r2 += `<th colspan="${c+1}" class="th-t${t}si sc${t}">${eu.name}<br><small>${Number(eu.max)||0} คะแนน</small></th>`;
            (eu.items||[]).forEach(item => r3 += `<th class="th-t${t}raw sc${t}">${item.name}<br><small>(${Number(item.max)||0})</small></th>`);
            r3 += `<th class="th-t${t}sc sc${t}">รวม</th>`;
          }
        });
      } else {
      ...
      // จบแค่ "สอบรวม"
      r1 += `<th rowspan="3" class="th-t${t}e" style="width:100px;background:#0369a1;color:#fff;">สอบรวม<br><small>(${ScoreLogic.getExamMax(t)})</small></th>`;

    } else {
      // หน่วย X — จบแค่รวมหน่วย
      ...
      // จบแค่ "รวมหน่วย" — ไม่มีเก็บรวม สอบ รวม เกรด
    }
  }

  // เพิ่ม รวม+เกรด เฉพาะ tab ทั้งหมด (uIdx===null) หรือ ทั้งปี
  const uIdxFinal = (App.activeTerm === 0) ? null : (App.activeUnit ?? null);
  const _isMathGrade = ['ม.1','ม.2','ม.3'].some(c => String(App.loadedClass||'').startsWith(c));
  if (App.activeTerm === 0 || uIdxFinal === null) {
    ...
    // มัธยม: มีคอลัมน์ ติด ร ก่อนเกรด
    if (_isMathGrade && !isYear) {
      r1 += `<th rowspan="3" style="width:72px;background:#dc2626;color:#fff;font-size:.8rem;">ติด ร<br>แก้ ร</th>`;
    }
    r1 += `<th rowspan="3" style="width:65px;background:#4f46e5;color:#fff;font-size:15px;vertical-align:middle;">เกรด</th>`;
  }
...
function buildBody() {
  ...
  const buildCells = (s) => {
    ...
    const examRaw = s.grades?.[`t${t}_exam`];
    const exam  = Number(examRaw) || 0;
    const _rawTotal = keep + exam;
    const total = isNaN(_rawTotal) ? 0 : _rawTotal;
    // hasAnyScore: มีข้อมูลจริง (ไม่ว่าง)
    const hasAnyScore = flatVals.some(v => v !== '' && v != null && v !== 0)
      || (examRaw !== '' && examRaw != null && examRaw !== undefined);
    const totalDisplay = hasAnyScore ? total : '-';
    const totalBadge = `<span class="tbadge ${!hasAnyScore?'nil':total>=(tgt*0.5)?'ok':'fail'} t${t}tot">${totalDisplay}</span>`;
    ...
    if (uIdx === -1) {
      // ── สอบ — สอบย่อย + รวมสอบ ──
      const examUnits = App.examUnits?.[t] || [];
      let examFlatIdx = 0, examScaledTotal = 0;
      const examFlat = s.grades?.[`t${t}_exam_sub`] || [];
      examUnits.forEach((eu, ei) => {
        const items = eu.items || []; let raw = 0;
        items.forEach((item) => {
          const val = examFlat[examFlatIdx] ?? '';
          raw += Number(val)||0;
          cells += `<td class="td-sub${t} sc${t}"><input type="number" class="sinput exam-sub${t}" min="0" max="${Number(item.max)||0}" value="${val}" data-term="${t}" data-examflatidx="${examFlatIdx}" oninput="calcExamSub(this,${t})"></td>`;
          examFlatIdx++;
        });
        const scaled = ScoreLogic.calcScaled(raw, ScoreLogic.getExamRawMax(t,ei), Number(eu.max)||0);
        examScaledTotal += scaled;
        const _hasExamData = items.some((_,ii2) => (examFlat[examFlatIdx - items.length + ii2]??'') !== '');
        cells += `<td class="td-sc${t} sc${t} exam-unit-total-${t}" data-examidx="${ei}">${_hasExamData ? scaled : '-'}</td>`;
      });
      ...
      const _sRawMax  = ScoreLogic.getUnitRawMax(t, uIdx);
      const _sScMax   = Number(unit?.max) || 0;
      const unitTotal = (_sRawMax > 0 && _sScMax > 0) ? ScoreLogic.calcScaled(raw, _sRawMax, _sScMax) : 0;
      const hasUnitData = items.some((_,i2) => (flatVals[base+i2]??'') !== '');
      if (items.length>0) cells += `<td class="td-sc${t} sc${t} unit-total-${t}" data-unit="${uIdx}">${hasUnitData ? unitTotal : '-'}</td>`;
      return { cells, keep: 0, exam: 0, total: unitTotal };

    } else {
      // ── ทั้งหมด — ทุกหน่วย + เก็บ + สอบ + รวม ──
      ScoreLogic.getUnitOffsets(t).forEach(({ unitIndex, start, count }) => {
        ...
        const _rawMax  = ScoreLogic.getUnitRawMax(t, unitIndex);
        const _scMax   = Number(unit?.max) || 0;
        const _scaled  = (_rawMax > 0 && _scMax > 0) ? ScoreLogic.calcScaled(raw, _rawMax, _scMax) : 0;
        // แสดง 0 ถ้ามีการกรอกจริง แม้คะแนนจะเป็น 0
        let _hasUData = false;
        for (let _ci = 0; _ci < count; _ci++) { if ((flatVals[start+_ci]??'') !== '') { _hasUData = true; break; } }
        cells += `<td class="td-sc${t} sc${t} unit-total-${t}" data-unit="${unitIndex}">${_hasUData ? _scaled : '-'}</td>`;
      });
      cells += `<td class="td-sc${t}"><span class="t${t}sc">${keep||'-'}</span></td>
                <td>${mkExamInput(s)}</td>
                <td>${totalBadge}</td>`;
    }
    return { cells, keep, exam, total, hasAnyScore };
  };

  $('gtBody').innerHTML = App.students.map((s, idx) => {
    const st = s.stats||{}, tc = buildCells(s), fs = tc.total;
    const showGrand = uIdx === null;
    const hasScore = tc.hasAnyScore !== undefined ? tc.hasAnyScore : fs > 0;
    return `<tr data-sid="${s.studentId}">
      <td class="s-c0">${idx+1}</td><td class="s-c1">${s.studentId}</td>
      ...
      ${tc.cells}
      ${showGrand ? (() => {
        const _isMG = ['ม.1','ม.2','ม.3'].some(c => String(App.loadedClass||'').startsWith(c));
        const isR = s.grades?.[`t${t}_is_r`] || false;
        const gradeVal = isR ? 'ร' : (!hasScore && fs===0 ? '-' : Utils.calcGradeFrontend(fs));
        const gradeCls = isR ? 'fail' : (!hasScore && fs===0 ? 'nil' : fs<tgt*0.5 ? 'fail' : 'ok');
        const rCol = (_isMG && !isYear) ? `
          <td style="text-align:center;background:${isR?'#fff1f2':'#f0fdf4'};padding:4px 6px;">
            <div class="r-switch-wrap ${isR?'is-r':''}"
              data-sid="${s.studentId}" data-term="${t}"
              onclick="toggleRStatus(this)"
              title="${isR?'ติด ร — คลิกเพื่อแก้ ร':'ปกติ — คลิกเพื่อติด ร'}"
              style="display:inline-flex;align-items:center;gap:5px;cursor:pointer;user-select:none;font-size:.75rem;font-weight:700;">
              <div style="
                position:relative;width:36px;height:18px;border-radius:9px;
                background:${isR?'#dc2626':'#22c55e'};
              ...
              <span style="color:${isR?'#dc2626':'#16a34a'};min-width:24px;">${isR?'ร':'✓'}</span>
            </div>
          </td>` : '';
        return `<td><span class="gbadge ${(!hasScore&&fs===0&&!isR)?'nil':(!isR&&fs<tgt*0.5)?'fail':'ok'} gtot">${(!hasScore&&fs===0&&!isR)?'-':fs}</span></td>
          ${rCol}
          <td style="background:#f8fafc;"><span class="gbadge ${gradeCls} final-grade ${isR?'r-grade':''}" style="font-size:1.05rem;border:2px solid ${isR?'#fecaca':'#c7d2fe'};min-width:48px;${isR?'background:#fee2e2;color:#dc2626;':''}">${gradeVal}</span></td>`;
      })() : ''}
    </tr>`;
  }).join('');
  refreshAllScoreInputStates($('gtBody'));
  if (typeof _updateRPreviewVisibility === 'function') _updateRPreviewVisibility();
}
...
function calcSub(el, t) {
  ...
  // sync กลับ App.students ทันที เพื่อให้ switchTab/rebuild ได้ค่าล่าสุด
  const _sid = tr.dataset.sid;
  const _stu = (App.students||[]).find(s => s.studentId === _sid);
  if (_stu) {
    if (!_stu.grades) _stu.grades = {};
    _stu.grades[`t${t}_sub`] = [...flatVals];
  }

  ScoreLogic.getUnitOffsets(t).forEach(({ unitIndex, start, count }) => {
    // ดึง inputs ของหน่วยนี้โดยตรงจาก data-unit
    const unitInputs = [...tr.querySelectorAll(`.sinput.sub${t}[data-unit="${unitIndex}"]`)];
    let raw = 0;
    unitInputs.forEach(inp => { raw += Number(inp.value) || 0; });
    // fallback: ถ้าไม่เจอ inputs ใช้ flatVals offset
    ...
    if (unitCell) {
      const _rawMax   = ScoreLogic.getUnitRawMax(t, unitIndex);
      const _scMax    = Number((App.units[t]||[])[unitIndex]?.max) || 0;
      const _uScaled  = ScoreLogic.calcScaled(raw, _rawMax, _scMax);
      const _uHasData = unitInputs.length
        ? unitInputs.some(i => i.value !== '')
        : Array.from({length: count}, (_,ci) => flatVals[start+ci]).some(v => v !== '' && v != null);
      unitCell.textContent = _uHasData ? _uScaled : '-';
    }
  });

  const scEl = tr.querySelector(`.t${t}sc`);
  if (scEl) {
    const _keep = ScoreLogic.calcKeepFromFlat(t, flatVals);
    const _hasAny = flatVals.some(v => v !== '' && v != null);
    scEl.textContent = _hasAny ? _keep : '-';
  }
...
function calcExam(el) {
  ...
  const tr = el.closest('tr');
  handleAutoTab(el, tr, mx);
  // sync กลับ App.students
  const _t  = Number(el.dataset.term) || App.activeTerm || 1;
  const _sid = tr.dataset.sid;
  const _stu = (App.students||[]).find(s => s.studentId === _sid);
  if (_stu) {
    if (!_stu.grades) _stu.grades = {};
    _stu.grades[`t${_t}_exam`] = el.value;
  }
  recalcTots(tr); refreshAllScoreInputStates(tr);
}
...
function calcExamSub(el, t) {
  ...
  // อัปเดต unit total ของ exam unit นั้น
  const examUnits = App.examUnits?.[t] || [];
  let offset = 0;
  const allInputs = [...tr.querySelectorAll(`.exam-sub${t}`)];
  examUnits.forEach((eu, ei) => {
    const items = eu.items || [];
    let raw = 0;
    items.forEach((_, ii) => { raw += Number(allInputs[offset+ii]?.value)||0; });
    const unitTotal = ScoreLogic.calcScaled(raw, ScoreLogic.getExamRawMax(t,ei), Number(eu.max)||0);
    const cell = tr.querySelector(`.exam-unit-total-${t}[data-examidx="${ei}"]`);
    if (cell) cell.textContent = unitTotal || '-';
    offset += items.length;
  });

  // คะแนนสอบรวม scale แล้ว
  const flatVals   = allInputs.map(i => Number(i.value)||0);
  const examScaled = ScoreLogic.calcExamScaled(t, flatVals);
...
function recalcTotsFromExamSub(tr, t, examTotal) {
  ...
  const tgt   = ScoreLogic.getTarget();
  const hasData = examTotal > 0 || keep > 0 ||
    [...tr.querySelectorAll(`.exam-sub${t}`)].some(i => i.value !== '');
  const disp = val => hasData ? val : (val > 0 ? val : '-');
  const upd  = (sel, val, cls) => { const e = tr.querySelector(sel); if(e){e.textContent=val;e.className=cls;} };
  upd(`.t${t}tot`, disp(total), `tbadge ${!hasData&&total===0?'nil':total>=(tgt*0.5)?'ok':'fail'} t${t}tot`);
  upd('.gtot',     disp(total), `gbadge ${!hasData&&total===0?'nil':total<tgt*0.5?'fail':'ok'} gtot`);
  const isR = tr.querySelector('.r-switch-wrap')?.classList.contains('is-r');
  const gradeVal = isR ? 'ร' : (!hasData && total===0 ? '-' : Utils.calcGradeFrontend(total));
  const gradeCls = isR ? 'fail' : (!hasData&&total===0 ? 'nil' : total<tgt*0.5 ? 'fail' : 'ok');
  upd('.final-grade', gradeVal, `gbadge ${gradeCls} final-grade`);
}

function recalcTots(tr) {
  const t = App.activeTerm || 1;
  const getVal = (sel) => Number(tr.querySelector(sel)?.textContent || tr.querySelector(sel)?.value || 0);
  const keep = getVal(`.t${t}sc`), exam = getVal(`.s-t${t}e`);
  const total = isNaN(keep + exam) ? exam : keep + exam;
  const tgt  = ScoreLogic.getTarget();
  // hasData: มีข้อมูลในช่องใดช่องหนึ่ง
  const examEl = tr.querySelector(`.s-t${t}e`);
  const hasData = (examEl && examEl.value !== '') ||
    [...tr.querySelectorAll(`.sinput.sub${t}`)].some(i => i.value !== '');
  const disp = val => hasData ? val : (val > 0 ? val : '-');
  const upd  = (sel, val, cls) => { const e = tr.querySelector(sel); if(e){e.textContent=val;e.className=cls;} };
  ...
  const gradeCls = isR ? 'fail' : (!hasData&&total===0 ? 'nil' : total<tgt*0.5 ? 'fail' : 'ok');
  upd('.final-grade', gradeVal, `gbadge ${gradeCls} final-grade`);
  const gs = tr.querySelector('.final-grade');
  if (gs) {
    if (isR) { gs.style.background='#fee2e2'; gs.style.color='#dc2626'; gs.style.border='2px solid #fecaca'; }
    else     { gs.style.background=''; gs.style.color=''; gs.style.border='2px solid #c7d2fe'; }
  }
}
...
// ── toggle สถานะ ติด ร / แก้ ร (switch) ───────────
function toggleRStatus(wrap) {
  const sid    = wrap.dataset.sid;
  const t      = Number(wrap.dataset.term);
  const wasR   = wrap.classList.contains('is-r');
  const willR  = !wasR;

  // ยืนยัน
  const stuName = (App.students||[]).find(s=>s.studentId===sid)?.name || sid;
  const msg = willR
    ? `⚠️ ติด ร นักเรียน: ${stuName}\nเทอม ${t}\n\nยืนยันการติด ร?`
    : `✅ แก้ ร นักเรียน: ${stuName}\nเทอม ${t}\n\nยืนยันการแก้ ร (เกรดจะคำนวณจากคะแนนจริง)?`;
  if (!confirm(msg)) return;

  // อัปเดต App.students snapshot
  const stu = (App.students||[]).find(s => s.studentId === sid);
  if (stu) {
    if (!stu.grades) stu.grades = {};
    stu.grades[`t${t}_is_r`] = willR;
  }

  // อัปเดต switch UI
  const thumb = wrap.querySelector('div > div');
  const track = wrap.querySelector('div');
  const label = wrap.querySelector('span');
  const td    = wrap.closest('td');

  if (willR) {
    wrap.classList.add('is-r');
    wrap.title = 'ติด ร — คลิกเพื่อแก้ ร';
    if (track) track.style.background = '#dc2626';
    if (thumb) thumb.style.left = '18px';
    if (label) { label.textContent = 'ร'; label.style.color = '#dc2626'; }
    if (td)    { td.style.background = '#fff1f2'; }
  } else {
    wrap.classList.remove('is-r');
    wrap.title = 'ปกติ — คลิกเพื่อติด ร';
    if (track) track.style.background = '#22c55e';
    if (thumb) thumb.style.left = '2px';
    if (label) { label.textContent = '✓'; label.style.color = '#16a34a'; }
    if (td)    { td.style.background = '#f0fdf4'; }
  }

  // อัปเดต badge เกรด
  const tr = wrap.closest('tr');
  if (!tr) return;
  const gradeSpan = tr.querySelector('.final-grade');
  if (!gradeSpan) return;

  if (willR) {
    gradeSpan.textContent = 'ร';
    gradeSpan.style.background = '#fee2e2';
    gradeSpan.style.color      = '#dc2626';
    gradeSpan.style.border     = '2px solid #fecaca';
    gradeSpan.classList.add('r-grade');
  } else {
    const totSpan = tr.querySelector('.gtot');
    const totVal  = totSpan ? Number(totSpan.textContent) || 0 : 0;
    const grade   = totVal === 0 ? '-' : Utils.calcGradeFrontend(totVal);
    const tgt     = ScoreLogic.getTarget();
    const cls     = totVal === 0 ? 'nil' : totVal < tgt * 0.5 ? 'fail' : 'ok';
    gradeSpan.textContent      = grade;
    gradeSpan.style.background = '';
    gradeSpan.style.color      = '';
    gradeSpan.style.border     = '2px solid #c7d2fe';
    gradeSpan.classList.remove('r-grade');
    gradeSpan.className = gradeSpan.className.replace(/(nil|ok|fail)/g, cls);
  }
  Utils.toast(willR ? `🔴 ติด ร: ${stuName} เทอม ${t}` : `🟢 แก้ ร: ${stuName} เทอม ${t}`);
}

// ── ดูเกรดจริง (ซ่อน ร ชั่วคราว) ─────────────────
App.rPreviewMode = false;

function toggleRPreview() {
  App.rPreviewMode = !App.rPreviewMode;
  const on     = App.rPreviewMode;
  const sw     = $('rPreviewSwitch');
  const thumb  = $('rPreviewThumb');
  const label  = $('rPreviewLabel');

  if (sw)    sw.style.background  = on ? '#6366f1' : '#94a3b8';
  if (thumb) thumb.style.left     = on ? '22px' : '2px';
  if (label) { label.textContent  = on ? 'เปิด' : 'ปิด'; label.style.color = on ? '#6366f1' : '#94a3b8'; }

  // re-render แค่คอลัมน์เกรดทุกแถว ไม่ต้อง rebuild ทั้งตาราง
  const t   = App.activeTerm || 1;
  const tgt = ScoreLogic.getTarget();
  document.querySelectorAll('#gtBody tr[data-sid]').forEach(tr => {
    const sid       = tr.dataset.sid;
    const stu       = (App.students || []).find(s => s.studentId === sid);
    if (!stu) return;
    const isR       = stu.grades?.[`t${t}_is_r`] || false;
    const gradeSpan = tr.querySelector('.final-grade');
    const rWrap     = tr.querySelector('.r-switch-wrap');
    if (!gradeSpan) return;

    if (on && isR) {
      // โหมดดูเกรดจริง: คำนวณเกรดจากคะแนน
      const totSpan = tr.querySelector('.gtot');
      const totVal  = totSpan ? Number(totSpan.textContent) || 0 : 0;
      const grade   = totVal === 0 ? '-' : Utils.calcGradeFrontend(totVal);
      const cls     = totVal === 0 ? 'nil' : totVal < tgt * 0.5 ? 'fail' : 'ok';
      gradeSpan.textContent      = grade;
      gradeSpan.style.background = '#ede9fe';
      gradeSpan.style.color      = '#6d28d9';
      gradeSpan.style.border     = '2px solid #c4b5fd';
      // ทำให้ switch ดูจางลงขณะ preview
      if (rWrap) rWrap.style.opacity = '0.4';
    } else if (!on && isR) {
      // ปิด preview: คืน ร
      gradeSpan.textContent      = 'ร';
      gradeSpan.style.background = '#fee2e2';
      gradeSpan.style.color      = '#dc2626';
      gradeSpan.style.border     = '2px solid #fecaca';
      if (rWrap) rWrap.style.opacity = '1';
    }
  });

  Utils.toast(on ? '👁 ดูเกรดจริง (ซ่อน ร ชั่วคราว)' : '🔴 คืนสถานะ ร แล้ว', on ? 'info' : 'warning');
}

// แสดง/ซ่อน rPreviewWrap ตามว่ามีนักเรียนติด ร ไหม
function _updateRPreviewVisibility() {
  const t    = App.activeTerm || 1;
  const hasR = (App.students || []).some(s => s.grades?.[`t${t}_is_r`]);
  const wrap = $('rPreviewWrap');
  if (wrap) wrap.style.display = hasR ? 'flex' : 'none';
  // ถ้าไม่มี ร แล้วให้ปิด preview mode ด้วย
  if (!hasR && App.rPreviewMode) {
    App.rPreviewMode = false;
    const sw = $('rPreviewSwitch'), thumb = $('rPreviewThumb'), label = $('rPreviewLabel');
    if (sw)    sw.style.background = '#94a3b8';
    if (thumb) thumb.style.left    = '2px';
    if (label) { label.textContent = 'ปิด'; label.style.color = '#94a3b8'; }
  }
}
...
function buildConfigPayload() {
  ...
  // preserve isSeq จาก config เดิมที่โหลดมาจาก server
  if (App.config && App.config.isSeq) payload.isSeq = true;
  return payload;
}
...
async function saveGrades() {
  ...
  $$('#gtBody tr[data-sid]').forEach(tr => {
    ...
    // อ่าน is_r จาก switch wrap
    const rWrap  = tr.querySelector('.r-switch-wrap');
    const isRVal = rWrap ? rWrap.classList.contains('is-r') : (snap[`t${t}_is_r`] || false);
    const oIsR   = snap[`t${other}_is_r`] || false;

    records.push(t === 1
      ? { studentId: sid, t1_sub: subVals, t1_acc: acc, t1_exam: examVal, t1_exam_sub: examSubVals, t1_is_r: isRVal,
          t2_sub: oSub, t2_acc: oAcc, t2_exam: oExam, t2_exam_sub: oExamSub, t2_is_r: oIsR }
      : { studentId: sid, t1_sub: oSub, t1_acc: oAcc, t1_exam: oExam, t1_exam_sub: oExamSub, t1_is_r: oIsR,
          t2_sub: subVals, t2_acc: acc, t2_exam: examVal, t2_exam_sub: examSubVals, t2_is_r: isRVal }
    );
  }); // end forEach tr
  ...
  try {
    ...
    records.forEach(rec => {
      if (stuMap[rec.studentId]) Object.assign(stuMap[rec.studentId], {
        t1_sub: rec.t1_sub, t1_acc: rec.t1_acc, t1_exam: rec.t1_exam, t1_exam_sub: rec.t1_exam_sub, t1_is_r: rec.t1_is_r,
        t2_sub: rec.t2_sub, t2_acc: rec.t2_acc, t2_exam: rec.t2_exam, t2_exam_sub: rec.t2_exam_sub, t2_is_r: rec.t2_is_r
      });
    });
...
let _saveConfigBusy = false;
async function saveConfigOnly() {
  if (_saveConfigBusy) return false;
  _saveConfigBusy = true;
  const subj = App.loadedSubject || $('gSubj').value;
  if (!subj || subj === "-- ไม่พบวิชา --") { _saveConfigBusy = false; return false; }
  if (ScoreLogic.getUnitsMax(1) > ScoreLogic.getTarget() || ScoreLogic.getUnitsMax(2) > ScoreLogic.getTarget()) { _saveConfigBusy = false; Utils.toast('คะแนนรวมเกินเป้าหมาย', 'warning'); return false; }
  Utils.showLoading('กำลังบันทึกการตั้งค่า...');
  try { await api('saveGrades', { year: App.loadedYear || $('gYear').value, classroom: App.loadedClass || $('gClass').value, subject: subj, config: buildConfigPayload(), gradeRecords:[] }); return true; } catch (e) { Utils.toast(e.message, 'error'); return false; } finally { Utils.hideLoading(); _saveConfigBusy = false; }
}
